...
class CCycleGAN():
    ...
    def build_generator(self):
        ...
        def conv2d(layer_input, filters, f_size=4,strides=2):
        ...
        d5 = conv2d(d4,  self.gf*8,strides=1)
        d6 = conv2d(d5,  self.gf*8,strides=1)

        #### 
        label = Input(shape=(1,), dtype='int32')
        label_embedding = Flatten()(Embedding(self.num_classes, np.prod(d6.get_shape().as_list()[1:]))(label))
        flat_img = Flatten()(d6)
        dmod = multiply([flat_img, label_embedding])
        dmod_resh = Reshape(d6.get_shape().as_list()[1:])(dmod)
        ####

        # Upsampling
        u1 = deconv2d(dmod_resh,   d5, self.gf*8,strides=2)
        u2 = deconv2d(u1,          d4, self.gf*8,strides=2)
        u3 = deconv2d(u2,          d3, self.gf*4)
        u4 = deconv2d(u3,          d2, self.gf*2)
        u5 = deconv2d(u4,          d1, self.gf)

        u8 = UpSampling2D(size=2)(u5)
...
if __name__ == '__main__':
    gan = CCycleGAN()
    gan.train(epochs=400, batch_size=64, sample_interval=400)
