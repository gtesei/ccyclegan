...
class CCycleGAN():
    ...
    def train(self, epochs, batch_size=1, sample_interval=50 , d_g_ratio=5, tun_grid=None):
        ...
        for epoch in range(epochs):
            for batch_i, (labels0 , imgs) in enumerate(self.data_loader.load_batch(batch_size=batch_size)):
                ...
                if batch_i % sample_interval == 0:
                    ...
                    ## ---> here measure and log FIS 
            ## FIS
            print(">>> measuring FIS ... ")
            self.measure_fis(epoch,tun_grid)
            

    def measure_fis(self, epoch,tun_grid,sample_size=1000):
        for i in range(self.num_classes):
                 _ , batch_images = self.data_loader.load_data(domain=None, batch_size=sample_size, is_testing=True,convertRGB=False)
                 _ , batch_images_i = self.data_loader.load_data(domain=i, batch_size=sample_size, is_testing=True,convertRGB=True)
                 for j in range(sample_size):
                     batch_images_i[j] = (batch_images_i[j]+1.)*127.5 #un-normalize
                 #batch_images = np.transpose(batch_images, (0,3,1,2) )
                 batch_images_i = np.transpose(batch_images_i, (0,3,1,2) )
                 #
                 labels1_all_i = to_categorical([i]*sample_size, num_classes=self.num_classes)
                 zs1_,zs2_,zs3_,zs4_ = self.g_enc.predict(batch_images)
                 _fake_i = self.g_dec.predict([zs1_,zs2_,zs3_,zs4_,labels1_all_i])
                 ...
                 tun_grid.loc[(tun_grid['d_gan_loss_w']==self.d_gan_loss_w) 
                     ...
                     & (tun_grid['rec_loss_w']==self.rec_loss_w)
                     & (tun_grid['adam_lr']==self.adam_lr)
                     & (tun_grid['epoch']==epoch) , 'lab'+str(i)+'_FIS' ]  = fid_i
        # store 
        tun_grid.to_csv(str(sys.argv[0]).split('.')[0]+'_fis.csv',index=False)
...
def generate_tuning_grid(MAX_EPOCH = 90, 
    ...
    tuned_parameters['epoch'] = list(range(MAX_EPOCH))
    #print([dict(zip(tuned_parameters.keys(),v)) for v in product(*tuned_parameters.values())])
    row_list = [dict(zip(tuned_parameters.keys(),v)) for v in product(*tuned_parameters.values())]
    row_dict = {i: row_list[i] for i in range(len(row_list)) }
    tun_grid = pd.DataFrame.from_dict(row_dict, orient='index')
    #
    #tun_grid['epoch'] = -1 
    #
    tun_grid['lab0_FIS'] = -1 
